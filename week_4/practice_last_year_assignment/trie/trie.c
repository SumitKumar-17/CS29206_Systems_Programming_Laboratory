...
int triesearch(trie *T, char *s)
{
    ...
    while (s[i] != '\0')
    {
        ...
        { // since string did not end and we have reached the root node
            return 0;
        }
        current = current->firstChild;

        while (current->nextSibling != NULL && current->char_label != s[i])
        { // traverse in same row
            current = current->nextSibling;
        }
        if (current->char_label != s[i])
        { // reached row end and still didn't find matching character
            return 0;
        }
        i++;
    }
    if (current->isword == 1)
    { // check whether the node is marked after finishing search
        return 1;
    }
    return 0;
}
...
void trieinsert(trie *T, char *s)
{
    ...
    {
        if (current->firstChild == NULL)
        { // if no child exists, we create a new child with the required character
        ...
        else
        {
            ...
            while (current->char_label < s[i] && current->nextSibling != NULL && current->nextSibling->char_label <= s[i])
            { // checks if required character is not in current along with the next sibling node
                current = current->nextSibling;
            }
            if (current->char_label > s[i])
            { // if the new character has to be inserted between current and previous node
                trienode *newnode = (trienode *)malloc(sizeof(trienode));
                newnode->char_label = s[i];
                newnode->isword = 0;
                newnode->firstChild = NULL;
            ...
            else if (current->char_label == s[i])
            { // if the current node has the character
                i++;
                continue;
            }
            else if (current->nextSibling != NULL && current->nextSibling->char_label > s[i])
            { // if the new character has to be inserted between current and next sibling node
                trienode *newnode = (trienode *)malloc(sizeof(trienode));
                newnode->char_label = s[i];
                newnode->isword = 0;
                newnode->firstChild = NULL;
...
void listall(trie *T, char *s)
{
    ...
    trienode *current = T->root;
    if (current->isword == 1)
    { // prints all the characters traversed down upto this node
        printf("%s\n", s);
    }
    current = current->firstChild;
    while (current != NULL)
